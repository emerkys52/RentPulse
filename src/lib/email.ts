...
const FROM_EMAIL = 'Rent Got Done <noreply@rentpulse.app>'
...
export async function sendVerificationEmail(email: string, token: string) {
  ...
  await getResend().emails.send({
    from: FROM_EMAIL,
    to: email,
    subject: 'Verify your Rent Got Done account',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #0f172a; color: #e2e8f0; padding: 40px 20px;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #1e293b; border-radius: 12px; padding: 40px;">
            <h1 style="color: #22d3ee; margin-bottom: 24px; font-size: 24px;">Welcome to Rent Got Done!</h1>
            ...
            <p style="color: #64748b; font-size: 12px;">Rent Got Done - Property Management Made Simple</p>
          </div>
        </body>
      </html>
    `,
...
export async function sendPasswordResetEmail(email: string, token: string) {
  ...
  await getResend().emails.send({
    from: FROM_EMAIL,
    to: email,
    subject: 'Reset your Rent Got Done password',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
...
export async function sendPremiumGrantedEmail(email: string, userName: string) {
  await getResend().emails.send({
    from: FROM_EMAIL,
    to: email,
    subject: 'Premium Access Granted - Rent Got Done',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #0f172a; color: #e2e8f0; padding: 40px 20px;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #1e293b; border-radius: 12px; padding: 40px;">
            ...
            <p style="margin-bottom: 24px; line-height: 1.6;">You've been granted Premium access to Rent Got Done! You now have access to all premium features including:</p>
...
export async function sendWelcomeEmail(email: string, userName: string) {
  await getResend().emails.send({
    from: FROM_EMAIL,
    to: email,
    subject: 'Welcome to Rent Got Done!',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #0f172a; color: #e2e8f0; padding: 40px 20px;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #1e293b; border-radius: 12px; padding: 40px;">
            <h1 style="color: #22d3ee; margin-bottom: 24px; font-size: 24px;">Welcome to Rent Got Done, ${userName}!</h1>
            <p style="margin-bottom: 24px; line-height: 1.6;">Thank you for joining Rent Got Done. We're excited to help you manage your rental properties more efficiently.</p>
...
export async function sendTrialStartedEmail(email: string, userName: string) {
  await getResend().emails.send({
    from: FROM_EMAIL,
    to: email,
    subject: 'Your 7-Day Premium Trial Has Started - Rent Got Done',
    html: `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #0f172a; color: #e2e8f0; padding: 40px 20px;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #1e293b; border-radius: 12px; padding: 40px;">
            ...
            <p style="margin-bottom: 24px; line-height: 1.6;">Hi ${userName}, you now have full access to all Rent Got Done Premium features for the next 7 days.</p>
            <div style="background-color: #0f172a; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
